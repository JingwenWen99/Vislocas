...
def get_metrics(classifier_model="Vislocas_mlce_lr-000005_bn_drop-01_attn-drop-01_drop-path-01_batch12_seed6293_wd-005_aug_no-normalized", thresh_func="t=0.5", thresh_func_prefix=""):
    args = parse_args()
    cfg = get_cfg()

    for database in cfg.DATA.DATASET_NAME:
        proteinLevelList = [False]
        if database in ["GraphLoc", "laceDNN"]:
            proteinLevelList = [True]
        if database in ["IHC"]:
            proteinLevelList = [True, False]
        for proteinLevel in proteinLevelList:
            multiInstance = "protein"
            ...
            if proteinLevel:
                f_path = "{}/results/{}/{}{}Level_crossValidation_{}_metrics.csv".format(cfg.DATA.RESULT_DIR, database, thresh_func_prefix, multiInstance, classifier_model)
            ...
            for split_num in split_list:
                ...
                for fold in range(total_fold):
                    ...
                    if proteinLevel:
                        val_path = "{}/{}/preds/{}Level/mean_test_{}_aug0_{}".format(result_prefix, classifier_model, multiInstance, thresh_func, val_file_path.split('/')[-1])
                    ...
                    if not database in ["SIN-Locator"] and split_num != -2:
                        ...
                        if proteinLevel:
                            test_path = "{}/{}/preds/{}Level/mean_test_{}_aug0_{}".format(result_prefix, classifier_model, multiInstance, thresh_func, test_file_path.split('/')[-1])
                ...
                if proteinLevel:
                    save_path = "{}/{}Level_crossValidation_{}_split{}.csv".format(result_prefix, multiInstance, thresh_func, split_num)
...
def cal_mean_std(classifier_model="Vislocas_mlce_lr-000005_bn_drop-01_attn-drop-01_drop-path-01_batch12_seed6293_wd-005_aug_no-normalized", thresh_func="t=0.5", thresh_func_prefix=""):
    args = parse_args()
    cfg = get_cfg()
    ...
    for database in cfg.DATA.DATASET_NAME:
        ...
        for proteinLevel in proteinLevelList:
            ...
            if proteinLevel:
                metricsPath = "{}/results/{}/{}{}Level_crossValidation_{}_metrics.csv".format(cfg.DATA.RESULT_DIR, database, thresh_func_prefix, multiInstance, classifier_model)
            ...
            if proteinLevel:
                metricsSavePath = "{}/results/{}/statistic_{}{}Level_crossValidation_{}_metrics.csv".format(cfg.DATA.RESULT_DIR, database, thresh_func_prefix, multiInstance, classifier_model)
