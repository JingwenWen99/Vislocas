...
if __name__ == '__main__':
    ...
    tissueData = pd.read_csv(dataDir + "normal_undownload.csv", header=None)
    print("原始Tissue数据条数：", len(tissueData))
    tissueData = tissueData.drop(tissueData.loc[tissueData[4].str.rsplit(".", expand=True, n=1)[1] == 'tif'].index)
    print("去除tif后，Tissue数据条数：", len(tissueData))
    tissueData.to_csv(dataDir + "normal_undownload.csv", header=None, index=None, mode='w')
